...
figure('Name', 'Exercise 2a — Lyapunov Driving Signal', ...
       'NumberTitle', 'off', 'Position', [80, 100, 900, 400]);

plot(t, s_vec, 'k', 'LineWidth', 1.2);
xlabel('Time $t$ [s]', 'Interpreter', 'latex');
ylabel('$s(t) = \dot{\theta}(t) - \hat{\dot{\theta}}(t)$ [rad/s]', 'Interpreter', 'latex');
title('Lyapunov Driving Signal $s(t)$: Velocity Tracking Error', 'Interpreter', 'latex');
yline(0, 'r--', 'LineWidth', 0.8);
grid on;

sgtitle('Exercise 2a: Lyapunov Method — Velocity Tracking Error $s(t)$ (no noise)', ...
...
%% Exercise 2b — Plots
% Plot 2b-1: Parameter Errors vs Noise Amplitude
figure('Name', 'Exercise 2b — Errors vs Noise Amplitude', ...
       'NumberTitle', 'off', 'Position', [80, 100, 900, 600]);

subplot(2, 1, 1);
semilogx(n0_values(2:end), error_l(2:end), 'bo-', 'LineWidth', 1.5, 'MarkerSize', 7);
hold on;
yline(error_l(1), 'r--', 'LineWidth', 1.5);
xlabel('Noise amplitude $\eta_0$', 'Interpreter', 'latex');
ylabel('$|l - \hat{l}|$ [m]', 'Interpreter', 'latex');
title('Estimation Error: pendulum length $l$', 'Interpreter', 'latex');
legend('With noise', 'Noise-free ($\eta_0 = 0$)', 'Interpreter', 'latex', 'Location', 'best');
grid on;

subplot(2, 1, 2);
semilogx(n0_values(2:end), error_c(2:end), 'ro-', 'LineWidth', 1.5, 'MarkerSize', 7);
hold on;
yline(error_c(1), 'b--', 'LineWidth', 1.5);
xlabel('Noise amplitude $\eta_0$', 'Interpreter', 'latex');
ylabel('$|c - \hat{c}|$', 'Interpreter', 'latex');
title('Estimation Error: damping coefficient $c$', 'Interpreter', 'latex');
legend('With noise', 'Noise-free ($\eta_0 = 0$)', 'Interpreter', 'latex', 'Location', 'best');
grid on;

sgtitle('Exercise 2b: Lyapunov Method — Estimation Error vs Noise Amplitude $\eta_0$', ...
        'FontSize', 13, 'FontWeight', 'bold', 'Interpreter', 'latex');

% Plot 2b-2: Clean vs Noisy Measurements (highest noise level)
eta0_demo  = n0_values(end);
noise_demo = eta0_demo * sin(20 * pi * t);
t_plot_idx = t <= 5;   % show first 5 seconds for clarity

figure('Name', 'Exercise 2b — Clean vs Noisy Measurements', ...
       'NumberTitle', 'off', 'Position', [80, 100, 900, 600]);

subplot(2, 1, 1);
plot(t(t_plot_idx), theta(t_plot_idx), 'b', 'LineWidth', 1.5); hold on;
plot(t(t_plot_idx), theta(t_plot_idx) + noise_demo(t_plot_idx), 'r.', 'MarkerSize', 6);
xlabel('Time $t$ [s]', 'Interpreter', 'latex');
ylabel('$\theta(t)$ [rad]', 'Interpreter', 'latex');
title(sprintf('Clean vs Noisy $\\theta(t)$ ($\\eta_0 = %.2f$)', eta0_demo), ...
      'Interpreter', 'latex');
legend('Clean $\theta(t)$', 'Noisy $\theta(t)$', 'Interpreter', 'latex', 'Location', 'best');
grid on;

subplot(2, 1, 2);
plot(t(t_plot_idx), theta_dot(t_plot_idx), 'b', 'LineWidth', 1.5); hold on;
plot(t(t_plot_idx), theta_dot(t_plot_idx) + noise_demo(t_plot_idx), 'r.', 'MarkerSize', 6);
xlabel('Time $t$ [s]', 'Interpreter', 'latex');
ylabel('$\dot{\theta}(t)$ [rad/s]', 'Interpreter', 'latex');
title(sprintf('Clean vs Noisy $\\dot{\\theta}(t)$ ($\\eta_0 = %.2f$)', eta0_demo), ...
      'Interpreter', 'latex');
legend('Clean $\dot{\theta}(t)$', 'Noisy $\dot{\theta}(t)$', ...
       'Interpreter', 'latex', 'Location', 'best');
grid on;

sgtitle(sprintf('Exercise 2b: Clean vs Noisy Measurements ($\\eta_0 = %.2f$)', eta0_demo), ...
        'FontSize', 13, 'FontWeight', 'bold', 'Interpreter', 'latex');

% Plot 2b-3: Parameter Convergence — No noise vs Low vs High
eta0_low  = 0.01;
eta0_high = 0.10;

[alpha_low,  c_low,  ~, ~] = lyapunov_estimator(t, ...
    theta + eta0_low  * sin(20*pi*t), theta_dot + eta0_low  * sin(20*pi*t), ...
    u + eta0_low  * sin(20*pi*t), gamma_alpha, gamma_c, alpha0, c0);
[alpha_high, c_high, ~, ~] = lyapunov_estimator(t, ...
    theta + eta0_high * sin(20*pi*t), theta_dot + eta0_high * sin(20*pi*t), ...
    u + eta0_high * sin(20*pi*t), gamma_alpha, gamma_c, alpha0, c0);

l_low  = g ./ alpha_low;
l_high = g ./ alpha_high;

figure('Name', 'Exercise 2b — Convergence: No noise vs Low vs High', ...
       'NumberTitle', 'off', 'Position', [80, 100, 900, 600]);

subplot(2, 1, 1);
plot(t, l_hat,  'b',  'LineWidth', 1.5); hold on;
plot(t, l_low,  'g--','LineWidth', 1.5);
...
legend('No noise', sprintf('$\\eta_0 = %.2f$', eta0_low), ...
       sprintf('$\\eta_0 = %.2f$', eta0_high), 'True $l$', ...
       'Interpreter', 'latex', 'Location', 'best');
grid on;

subplot(2, 1, 2);
plot(t, c_hat,  'b',  'LineWidth', 1.5); hold on;
plot(t, c_low,  'g--','LineWidth', 1.5);
plot(t, c_high, 'r:', 'LineWidth', 1.8);
yline(c, 'k-', 'LineWidth', 1.2);
xlabel('Time $t$ [s]', 'Interpreter', 'latex');
ylabel('$\hat{c}(t)$', 'Interpreter', 'latex');
title('Damping Estimate $\hat{c}(t)$ for Different Noise Levels', 'Interpreter', 'latex');
legend('No noise', sprintf('$\\eta_0 = %.2f$', eta0_low), ...
       sprintf('$\\eta_0 = %.2f$', eta0_high), 'True $c$', ...
       'Interpreter', 'latex', 'Location', 'best');
grid on;

sgtitle('Exercise 2b: Lyapunov Method — Parameter Convergence vs Noise Level', ...
        'FontSize', 13, 'FontWeight', 'bold', 'Interpreter', 'latex');
